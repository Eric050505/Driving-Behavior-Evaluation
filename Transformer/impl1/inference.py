...
def run_encoder_decoder_inference(
    ...
    if batch_first:
        tgt = src[:, -1:, :]  # [batch_size, 1, input_size], e.g., [32, 1, 1]
    else:
        tgt = src[-1:, :, :]
    ...
    for _ in range(forecast_window-1):
        ...
        tgt_mask = utils.generate_square_subsequent_mask(dim1=dim_a, dim2=dim_a).to(device)
        src_mask = utils.generate_square_subsequent_mask(dim1=dim_a, dim2=dim_b).to(device)
